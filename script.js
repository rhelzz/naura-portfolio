...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Brand Ambassador stats counter
    const brandStatsCounters = document.querySelectorAll('.brand-stats .counter');
    const brandStatsObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                const counter = entry.target;
                ...
                const updateCount = () => {
                    const increment = target / 80; // Speed kontroler
                ...
                updateCount();
                brandStatsObserver.unobserve(counter);
            }
        });
    }, {threshold: 0.5});

    brandStatsCounters.forEach(counter => {
        brandStatsObserver.observe(counter);
    });
